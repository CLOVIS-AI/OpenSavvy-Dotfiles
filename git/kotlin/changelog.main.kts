...
sealed class Printer {
	...
	object Terminal : Printer() {
		...
		override fun title(message: String) = println("${escape(1)}$message${escape(0)}  ")
		override fun text(message: String) = println(message)
		override fun item(message: String) = println("• $message")
		override fun url(message: String, url: String) = println("$message ($url)")
	}
...
fun Commit.findType(): Pair<Type, Commit> = with(subject) {
	when {
		startsWith("feat") -> Type.FEAT to withScope()
		startsWith("New") -> Type.FEAT to withScope()

		startsWith("build") -> Type.BUILD to withScope()
		startsWith("Build") -> Type.BUILD to withScope()

		startsWith("upgrade") -> Type.UPGRADE to withScope()
		startsWith("Upgrade") -> Type.UPGRADE to withScope()

		startsWith("ci") -> Type.CI to withScope()

		startsWith("fix") -> Type.FIX to withScope()
		startsWith("fixes") -> Type.FIX to withScope()
		startsWith("Fix") -> Type.FIX to withScope()

		startsWith("doc") -> Type.DOC to withScope()
		startsWith("docs") -> Type.DOC to withScope()
		startsWith("Docs") -> Type.DOC to withScope()

		startsWith("perf") -> Type.PERF to withScope()
		startsWith("perfs") -> Type.PERF to withScope()

		startsWith("refactor") -> Type.REFACTOR to withScope()
		startsWith("Update") -> Type.REFACTOR to withScope()

		startsWith("style") -> Type.STYLE to withScope()

		startsWith("test") -> Type.TEST to withScope()
		startsWith("tests") -> Type.TEST to withScope()

		startsWith("breaking") -> Type.BREAKING to withScope()
		startsWith("Breaking") -> Type.BREAKING to withScope()

		startsWith("Merge branch ") -> Type.MERGE to this@findType
		startsWith("merge") -> Type.MERGE to withScope()

		else -> Type.UNKNOWN to this@findType
	}
}
